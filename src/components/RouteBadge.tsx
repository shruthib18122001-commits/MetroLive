...
function toneFor(routeId: string, routeName: string): string {
  ...
  if (/\bb line\b|\b802\b/.test(key)) return 'bg-red-600 text-white';
  if (/\bc line\b|\b803\b/.test(key)) return 'bg-green-600 text-white';
  ...
  if (/\bk line\b|\b810\b/.test(key)) return 'bg-pink-500 text-white';
  if (/\bg line\b|\bj line\b|\b901\b|\b910\b/.test(key)) return 'bg-orange-500 text-white';
